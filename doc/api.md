...
| `.export("out.step")` | Write file; format determined by extension: `.step`/`.stp` → STEP, `.stl` → STL, `.3mf` → 3MF (units, colour, one object per solid), `.glb` → GLB, `.gltf` → glTF, `.obj` → OBJ, `.svg` → SVG 2-D drawing, `.dxf` → DXF R12 2-D drawing. Anything unrecognised is written as STEP. Tessellation is fixed at 0.1 mm linear deflection; a `linear_deflection:` option is accepted and ignored. |
