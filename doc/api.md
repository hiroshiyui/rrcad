...
| `.export_dxf(path: &str, view: &str, scale: f64, hidden: bool) -> Result<()>` | DXF R12 ASCII drawing. Same `view`, `scale`, and `hidden` options as `export_svg`; hidden entities are written on a `HIDDEN` layer. |
...
| `.export("out.dxf", view: :top\|:front\|:side, scale: 1.0, hidden: false)` | DXF R12 ASCII drawing via the same HLR pipeline. `scale:` multiplies drawing geometry and must be positive. `hidden: true` writes hidden `LINE` entities on a `HIDDEN` layer. Outputs Y-up CAD coordinates. |
