...
| `.export_3mf(path: &str, linear_deflection: f64) -> Result<()>` | 3MF. Two-stage, because OCCT has no 3MF writer: C++ (`shape_3mf_model`) tessellates and returns the `3D/3dmodel.model` XML, then Rust (`threemf::write_package`) wraps it in the OPC ZIP. One `<object>` per solid, welded on exact node coordinates and wound outward; `unit="millimeter"`; colour from `Shape#color` as a `<basematerials>` entry. Errors if the shape tessellates to no triangles. |
...
part.export_stl("/tmp/part.stl", 0.1, false)?;   // false = binary
...
fn export_stl (shape: &OcctShape, path: &str, linear_deflection: f64,
               ascii: bool)                                           -> Result<()>;
...
// 3MF returns the model XML instead of writing a file: a 3MF is a ZIP, and the
// ZIP library is on the Rust side.
...
| `text("X-450", size: 6, font: nil)` | Glyph outlines of the string as a Compound of planar Faces in the XY plane, baseline at the origin, `size:` tall in mm. Extrude for solid letters, then fuse to emboss or cut to engrave. `font:` names a family (system font manager) or a `.ttf`/`.otf` path; omitted, the sans-serif default. Uses `Font_BRepFont` + `Font_BRepTextBuilder` (TKService/TKV3d). Rebuild re-renders the stored request, so the font must still resolve then. |
| `airfoil(naca: "2412", chord: 20, samples: 40)` | Closed aerofoil Face in the XY plane: leading edge at the origin, chord along +X, upper surface toward +Y. Two interpolated BSpline segments (upper/lower surface) meeting at a sharp trailing edge, so the section stays smooth through `extrude`/`loft`. NACA 4-digit sections are generated analytically with the closed-trailing-edge polynomial and cosine point spacing; `samples:` is points per surface. Pure Ruby DSL over `make_profile_2d`. |
| `airfoil(coordinates: [[x,y], ...], chord: nil)` | Same Face from explicit points in Selig order (trailing edge → upper surface → leading edge → lower surface → trailing edge). `chord:` rescales the X extent and shifts the leading edge to x = 0; omitted, the data is used as-is. A blunt trailing edge (first ≠ last point) is closed with a straight base. |
| `airfoil(dat: "<file text>", chord: nil)` | Same Face from the contents of a Selig-format `.dat` file passed as a String (a leading name line is skipped; consecutive duplicate points are dropped). Lednicer-ordered files are detected and rejected with a pointer to Selig order. |
| `loft([profile1, profile2, ...])` | Loft through a sequence of circle/sketch profiles; `ruled: false` (default) gives smooth blending |
| `sweep_sections(path, [profile1, profile2, ...], twist: nil, scale: nil)` | Variable-section sweep: each origin-centred profile is automatically placed at the corresponding spine point and swept along `path` (a `spline_3d` Wire). Uses `BRepOffsetAPI_MakePipeShell`; falls back to `ThruSections` loft for highly-curved paths. Supports `circle`, `rect`, `polygon`, `ellipse`, `arc`, and `airfoil` profiles. `twist:` rotates each profile in its own plane before placement — a Numeric is the total twist in degrees blended linearly from 0, an Array gives one angle per profile. `scale:` resizes each profile about its origin — a Numeric is the end scale blended linearly from 1, an Array one positive factor per profile. Both keywords are applied in the prelude wrapper over the native `__rrcad_sweep_sections`; profiles are transformed as copies, never mutated. |
...
| `nut_pocket(size, depth:, style: :hex, clearance: 0.2, slot: nil)` | Recess tool for a captive nut in a printed part: a hex (or `:square`) prism centred on the origin, extruded +Z from Z=0, sized to the nut's across-flats plus 2 × `clearance:`. `slot:` extends the pocket along +Y by that length so the nut slides in from an edge (flats face ±X, so the channel guides the nut on its flats). Same metric/imperial sizes as `nut`; a numeric size is a nominal thread diameter (across-flats ≈ 1.7×). Subtract with `.cut`; pair with `clearance_hole` for the screw. Pure Ruby DSL. |
| `standoff_pocket(size, depth:, clearance: 0.2)` | Hex recess that keeps a threaded hex standoff from spinning in a frame plate. Standoffs share the across-flats of the nut of their thread, so this is `nut_pocket` under the name of its use case. Pure Ruby DSL. |
...
| `.shell(thickness, open: nil)` | Hollow out a solid, leaving walls of `thickness`. Without `open:` the topmost face is removed, as always. `open:` chooses the opening(s): a Face from `.faces` on this same shape, an Array of them, or a `.faces` selector (`:top`, `:bottom`, `:side`, `:">X"`, …). Faces are matched against the body by `IsSame`, so a face of another shape (or of a transformed copy) is rejected by name; selecting every face is refused. Rebuild re-finds the openings by face centroid. |
...
| `.export("out.step")` | Write file; format determined by extension: `.step`/`.stp` → STEP, `.stl` → STL, `.3mf` → 3MF (units, colour, one object per solid), `.glb` → GLB, `.gltf` → glTF, `.obj` → OBJ, `.svg` → SVG 2-D drawing, `.dxf` → DXF R12 2-D drawing. Anything unrecognised is written as STEP. |
| `.export("out.stl", ascii: true)` | `.stl` only: write the text encoding instead of binary. Binary is the default — same triangles, roughly a fifth of the bytes, and read by every slicer. Use text for diffing an export or for a tool that cannot read binary. Ignored by every other format. |
| `.export("out.stl", linear_deflection: 0.05)` | Mesh formats only (`.3mf`, `.stl`, `.glb`, `.gltf`, `.obj`): the largest gap allowed between the mesh and the true surface, in mm. Default 0.1; must be > 0; `deflection:` is accepted as a shorter spelling. Ignored by STEP and the 2-D drawing formats. Re-exporting a shape at a different deflection re-meshes it. |
...
| `asm.export(path, **opts)` | Fuse all shapes and export to file. When declarative parts are present, the assembly is solved lazily before fusion. Options are forwarded untouched to `Shape#export`, so an assembly takes the same drawing controls a part does (`view:`, `scale:`, `section:`, `dimensions:`, `ordinate:`, `detail:`, `hidden:`, `title_block:`). `bom: true` adds the bill of materials as a table below the drawing (SVG `bom` group / DXF `BOM` layer); `balloons: true` adds one numbered callout per parts-list row, keyed to its item number, with a leader landing on that component (SVG `balloons` group / DXF `BALLOON` layer). Both are drawing-only and ignored by the solid formats. `structured: true` (STEP only) writes the components as separate named PRODUCTs under one root assembly instead of fusing — via XCAF + `STEPCAFControl_Writer` — so FreeCAD/Fusion reopen the parts individually. Component names come from `name:` (auto `part_N`), the assembly name from `assembly("…")`, and a part's `.color` travels as `COLOUR_RGB`/`STYLED_ITEM`. Geometry stays in world coordinates (identity component transforms, no instance reuse). |
