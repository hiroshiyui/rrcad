...
| `.export_svg(path: &str, view: &str, scale: f64, hidden: bool, center_marks: bool, dimensions: bool) -> Result<()>` | SVG 2-D drawing. `view` is `"top"` (default), `"front"`, or `"side"`; `scale` defaults to `1.0` and must be positive; `hidden` includes dashed hidden-line geometry; `center_marks` adds cylinder centre marks; `dimensions` adds overall width/height labels. Uses `HLRBRep_PolyAlgo` + `HLRBRep_PolyHLRToShape`. |
| `.export_dxf(path: &str, view: &str, scale: f64, hidden: bool, center_marks: bool, dimensions: bool) -> Result<()>` | DXF R12 ASCII drawing. Same `view`, `scale`, `hidden`, `center_marks`, and `dimensions` options as `export_svg`; hidden entities are written on a `HIDDEN` layer, center marks on a `CENTER` layer, and dimensions on a `DIMENSION` layer. |
...
| `.export("out.svg", view: :top\|:front\|:side, scale: 1.0, hidden: false, center_marks: false, dimensions: false)` | SVG 2-D drawing via `HLRBRep_PolyAlgo` hidden-line removal. `:top` (default) looks down −Z; `:front` looks along −Y; `:side` looks along +X. `scale:` multiplies drawing geometry and must be positive. `hidden: true` adds dashed hidden-line polylines. `center_marks: true` adds cylinder centre marks. `dimensions: true` adds width/height labels. Outputs `<polyline>` elements with Y-down SVG coordinates. |
| `.export("out.dxf", view: :top\|:front\|:side, scale: 1.0, hidden: false, center_marks: false, dimensions: false)` | DXF R12 ASCII drawing via the same HLR pipeline. `scale:` multiplies drawing geometry and must be positive. `hidden: true` writes hidden `LINE` entities on a `HIDDEN` layer. `center_marks: true` writes centre marks on a `CENTER` layer. `dimensions: true` writes overall width/height labels on a `DIMENSION` layer. Outputs Y-up CAD coordinates. |
