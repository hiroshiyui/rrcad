...
| `.export_dxf(path: &str, view: &str, scale: f64) -> Result<()>` | DXF R12 ASCII drawing. Same `view` and `scale` options as `export_svg`. |
...
| `.export("out.dxf", view: :top\|:front\|:side, scale: 1.0)` | DXF R12 ASCII drawing via the same HLR pipeline. `scale:` multiplies drawing geometry and must be positive. Outputs `LINE` entities with Y-up CAD coordinates. |
