...
| `.export_dxf(path: &str, view: &str, scale: f64, hidden: bool, center_marks: bool) -> Result<()>` | DXF R12 ASCII drawing. Same `view`, `scale`, `hidden`, and `center_marks` options as `export_svg`; hidden entities are written on a `HIDDEN` layer and center marks on a `CENTER` layer. |
...
| `.export("out.dxf", view: :top\|:front\|:side, scale: 1.0, hidden: false, center_marks: false)` | DXF R12 ASCII drawing via the same HLR pipeline. `scale:` multiplies drawing geometry and must be positive. `hidden: true` writes hidden `LINE` entities on a `HIDDEN` layer. `center_marks: true` writes centre marks on a `CENTER` layer. Outputs Y-up CAD coordinates. |
