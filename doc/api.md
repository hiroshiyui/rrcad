...
| `.export_svg(path: &str, view: &str, scale: f64, hidden: bool, center_marks: bool, dimensions: bool, title_block: bool) -> Result<()>` | SVG 2-D drawing. `view` is `"top"` (default), `"front"`, `"side"`, or `"sheet"` for a 3-view sheet; `scale` defaults to `1.0` and must be positive; `hidden` includes dashed hidden-line geometry; `center_marks` adds cylinder centre marks; `dimensions` adds overall width/height labels; `title_block` adds a simple metadata block. Uses `HLRBRep_PolyAlgo` + `HLRBRep_PolyHLRToShape`. |
| `.export_dxf(path: &str, view: &str, scale: f64, hidden: bool, center_marks: bool, dimensions: bool, title_block: bool) -> Result<()>` | DXF R12 ASCII drawing. Same `view`, `scale`, `hidden`, `center_marks`, `dimensions`, and `title_block` options as `export_svg`; `view` also accepts `"sheet"` for a 3-view layout. Hidden entities are written on a `HIDDEN` layer, center marks on a `CENTER` layer, dimensions on a `DIMENSION` layer, and title block entities on `TITLEBLOCK`. |
