...
| `.export_svg(path: &str, view: &str, scale: f64, hidden: bool, center_marks: bool) -> Result<()>` | SVG 2-D drawing. `view` is `"top"` (default), `"front"`, or `"side"`; `scale` defaults to `1.0` and must be positive; `hidden` includes dashed hidden-line geometry; `center_marks` adds cylinder centre marks. Uses `HLRBRep_PolyAlgo` + `HLRBRep_PolyHLRToShape`. |
