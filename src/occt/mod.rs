...
enum FeatureOp {
    ...
    Torus { r1: f64, r2: f64 },
    Wedge { dx: f64, dy: f64, dz: f64, ltx: f64 },
    Rect { w: f64, h: f64 },
    Circle { r: f64 },
    Polygon { points: Vec<f64> },
    Ellipse { rx: f64, ry: f64 },
    Arc { r: f64, start_deg: f64, end_deg: f64 },
    Translate { dx: f64, dy: f64, dz: f64 },
    Rotate { axis_x: f64, axis_y: f64, axis_z: f64, angle_deg: f64 },
    Scale { factor: f64 },
    ScaleXyz { sx: f64, sy: f64, sz: f64 },
    Mirror { plane: String },
    Fuse,
    Cut,
    Common,
    ...
    Helix { radius: f64, pitch: f64, height: f64 },
    Loft { ruled: bool, profile_count: usize },
    Shell { thickness: f64 },
    Offset { distance: f64 },
    Offset2D { distance: f64 },
    Simplify { min_feature_size: f64 },
    Sweep,
    SweepGuide,
    ImportStep { path: String },
    ImportStl { path: String },
    PatternLinear { n: i32, dx: f64, dy: f64, dz: f64 },
    PatternPolar { n: i32, angle_deg: f64 },
    FragmentAll { count: usize },
    ConvexHull,
    PathPattern { n: i32 },
    Slice { plane: String, offset: f64 },
    Fillet { radius: f64 },
    FilletSel { radius: f64, selector: String },
    FilletVar { r1: f64, r2: f64 },
    FilletVarSel { r1: f64, r2: f64, selector: String },
    Chamfer { dist: f64 },
    ChamferSel { dist: f64, selector: String },
    ChamferAsym { d1: f64, d2: f64 },
    ChamferAsymSel { d1: f64, d2: f64, selector: String },
    Pad { height: f64 },
    Pocket { depth: f64 },
    FilletWire { radius: f64 },
    DatumPlane {
        ox: f64,
        oy: f64,
        oz: f64,
    ...
    ExtrudeDraft { height: f64, draft_deg: f64 },
    BezierPatch { points: Vec<f64> },
    Sew { face_count: usize, tolerance: f64 },
    SweepSections { profile_count: usize },
    RuledSurface,
    FillSurface,
    Opaque { label: String },
}
...
impl FeatureOp {
    fn name(&self) -> String {
        match self {
            ...
            FeatureOp::Arc { r, start_deg, end_deg } => {
            ...
            FeatureOp::Extrude {
                ...
                "extrude(height={height}, twist_deg={twist_deg}, scale={scale})"
            ),
            ...
            FeatureOp::Helix { radius, pitch, height } => {
...
impl FeatureNode {
    ...
    fn snapshot_lines(&self, out: &mut Vec<String>, seen: &mut BTreeSet<u64>) {
    ...
    fn rebuild(&self) -> Result<Shape, String> {
        match &self.op {
        ...
        ffi::pipe_shell_build(builder.pin_mut())
            .map(|p| {
                let profile_summary = profiles
                    ...
                    .collect::<Vec<_>>()
                    .join(", ");
                let mut parents: Vec<Arc<FeatureNode>> =
                    profiles.iter().map(|s| s.feature.clone()).collect();
    ...
    pub fn fragment_all(shapes: &[&Shape]) -> Result<Shape, String> {
        ...
        ffi::fragment_build(builder.pin_mut())
            .map(|p| {
                ...
                Shape::fresh_with_feature_parents(
                    p,
                    FeatureOp::FragmentAll { count: shapes.len() },
