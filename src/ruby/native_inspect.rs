...
shape_array!(
    ...
    /// least 7 doubles.
    rrcad_shape_cylinder_axis => cylinder_axis, len = 7
);

shape_flag!(
    /// Returns 1 if the shape is closed (every edge shared by ≥2 faces), 0 otherwise.
    /// Returns -1 and sets *error_out on error.
    rrcad_shape_is_closed => is_closed
);

shape_flag!(
    /// Returns 1 if the shape is manifold (every edge shared by exactly 2 faces), 0 otherwise.
    /// Returns -1 and sets *error_out on error.
    rrcad_shape_is_manifold => is_manifold
);

shape_array!(
    /// Fill `out[0..6]` with `[xmin, ymin, zmin, xmax, ymax, zmax]`.
    rrcad_shape_bounding_box => bounding_box, len = 6
);

shape_scalar!(rrcad_shape_volume() => volume, on_error = 0.0);
shape_scalar!(rrcad_shape_surface_area() => surface_area, on_error = 0.0);
shape_scalar!(rrcad_shape_distance_to(b_ptr: shape) => distance_to, on_error = f64::NAN);

shape_array!(
    /// Fill `out[0..6]` with `[ixx, iyy, izz, ixy, ixz, iyz]`.
    rrcad_shape_inertia => inertia, len = 6
);

shape_scalar!(rrcad_shape_min_thickness() => min_thickness, on_error = f64::NAN);

shape_string!(rrcad_shape_validate => validate);

// `history` / `feature_graph` never fail, so they do not fit `shape_string!`
// (which threads a `Result` through `*error_out`); they only need the
...
pub unsafe extern "C" fn rrcad_shape_history(
    ...
    unsafe { owned_str_ptr(&shape.history().join("\n")) }
}
