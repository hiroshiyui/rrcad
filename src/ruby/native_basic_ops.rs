...
use super::native_helpers::{set_err, utf8_arg};

// ---------------------------------------------------------------------------
// Boolean operations
// ---------------------------------------------------------------------------

shape_method!(rrcad_shape_fuse(b: shape) => fuse);
shape_method!(rrcad_shape_cut(b: shape) => cut);
shape_method!(rrcad_shape_common(b: shape) => common);

// ---------------------------------------------------------------------------
// Assembly mating (Phase 5)
// ---------------------------------------------------------------------------

shape_method!(rrcad_shape_mate(from_ptr: shape, to_ptr: shape, offset: f64) => mate);

// ---------------------------------------------------------------------------
// Color (Phase 5)
// ---------------------------------------------------------------------------

shape_method!(rrcad_shape_set_color(r: f64, g: f64, b: f64) => set_color);

// ---------------------------------------------------------------------------
// Named features (Phase 5)
//
// These three write no value back — they only report failure through
// `*error_out` — so they are spelled out rather than macro-generated.
// ---------------------------------------------------------------------------
...
pub unsafe extern "C" fn rrcad_shape_name_face(
    ...
    let Some(name) = (unsafe { utf8_arg(name, "name", error_out) }) else {
        return;
    };
...
shape_method!(rrcad_shape_ref(name: str) => ref_named);

// ---------------------------------------------------------------------------
// Transforms (Phase 2 — wiring existing OCCT ops)
// ---------------------------------------------------------------------------

shape_method!(rrcad_shape_translate(dx: f64, dy: f64, dz: f64) => translate);
shape_method!(rrcad_shape_rotate(ax: f64, ay: f64, az: f64, angle_deg: f64) => rotate);
shape_method!(rrcad_shape_scale(factor: f64) => scale);
shape_method!(rrcad_shape_scale_xyz(sx: f64, sy: f64, sz: f64) => scale_xyz);
shape_method!(rrcad_shape_fillet(radius: f64) => fillet);
shape_method!(rrcad_shape_chamfer(dist: f64) => chamfer);
shape_method!(rrcad_shape_fillet_sel(radius: f64, selector: str) => fillet_sel);
shape_method!(rrcad_shape_chamfer_sel(dist: f64, selector: str) => chamfer_sel);
shape_method!(rrcad_shape_fillet_var(r1: f64, r2: f64) => fillet_var);
shape_method!(rrcad_shape_fillet_var_sel(r1: f64, r2: f64, selector: str) => fillet_var_sel);

// ---------------------------------------------------------------------------
// Phase 7 Tier 1: asymmetric chamfer
// ---------------------------------------------------------------------------

shape_method!(rrcad_shape_chamfer_asym(d1: f64, d2: f64) => chamfer_asym);
shape_method!(rrcad_shape_chamfer_asym_sel(d1: f64, d2: f64, selector: str) => chamfer_asym_sel);
