...
1. **Verify the build is clean** — before anything else, run `./scripts/clean-build.sh` to confirm a from-scratch build passes. This mirrors CI (which never has a cached `libmruby.a`) and catches build-plumbing bugs that a normal `cargo build` would hide due to caching.
...
3. **Update the version** — bump the `version` field in `Cargo.toml` to match the new release version.
...
5. **Commit the release** — stage `Cargo.toml`, `Cargo.lock`, and `CHANGELOG.md` together and commit with the message `chore: release vX.Y.Z`.
