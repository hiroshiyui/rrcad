...
- **Phase 10** ◐ — Usability and robust parametric CAD: constraint-based sketching MVP (`sketch do … end` with `fixed`, `horizontal`, `vertical`, `coincident`, `dimension`, `equal_length`, `parallel`, `perpendicular`, `symmetric`, `mirror_x`/`mirror_y`, `tangent`, construction points/lines, `circle_at`/`arc_at`/`rectangle`/`centered_rectangle`/`slot_between`); units helpers (`1.6.mm`, `2.inch`, `15.deg`); hardware helpers (`clearance_hole`, `tap_drill`, `heat_set_insert`, `socket_head_cbore`, `flat_head_csink`, `bearing_bore`, `shaft`, `screw` for `:m2`–`:m5` and standard ball-bearing sizes); assembly constraint helpers (`Assembly#distance_mate`, `axis_align`, `angle_mate`, `Shape#rotate_about`)
